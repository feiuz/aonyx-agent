...
- **Vague 4** — 🚧 in progress: a REST + WebSocket **automation API** (`aonyx serve api` — sessions, streaming, memory, tools, OpenAI-compat, OpenAPI; ✅ v0.7.0); **real end-to-end tool calling** across every OpenAI-compatible, Anthropic, and Ollama provider — reaching the served paths, with a `tools_allow`/`tools_deny` policy (✅ v0.8.0–v0.8.1); and a **Windows desktop app** (Tauri 2 — streaming markdown chat, sessions, memory search; in-repo under [`desktop/`](desktop/), ships as v0.9.0).
